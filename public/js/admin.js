...
function renderAdminOrdersTable(orders) {
    const container = document.getElementById('adminOrdersContainer') || document.getElementById('adminOrdersTableBody');
    if (!container) return;
        ...
        container.innerHTML = `
            <div class="col-12 text-center py-5">
                <div class="empty-orders-card p-5 rounded-4">
                    ...
                    <h5 class="fw-bold text-ink mb-1">لا توجد طلبات جارية حالياً</h5>
                    <p class="text-muted small mb-0">جميع الطلبات الجديدة ستظهر هنا فور تسجيلها تلقائياً.</p>
                </div>
            </div>`;
        return;
    }
    ...
    const statusConfig = {
        'New': { label: 'جديد ⏳', bg: '#B5401A', color: '#FFFFFF', icon: 'fa-fire-burner' },
        'Reviewed': { label: 'تمت المراجعة 📋', bg: '#A8792F', color: '#FFFFFF', icon: 'fa-clipboard-check' },
        'Preparing': { label: 'جاري التحضير 🧑‍🍳', bg: '#D8B978', color: '#1B1512', icon: 'fa-kitchen-set' },
        'Ready': { label: 'جاهز بالـمطبخ 📦', bg: '#35586B', color: '#FFFFFF', icon: 'fa-box-open' },
        'OutForDelivery': { label: 'خرج للتوصيل 🛵', bg: '#1B1512', color: '#D8B978', icon: 'fa-motorcycle' },
        'Delivered': { label: 'تم التسليم ✅', bg: '#556B45', color: '#FFFFFF', icon: 'fa-circle-check' },
        'Cancelled': { label: 'تم الإلغاء ❌', bg: '#7A6C58', color: '#FFFFFF', icon: 'fa-circle-xmark' },
        'Rejected': { label: 'مرفوض 🚫', bg: '#2A2119', color: '#FFFFFF', icon: 'fa-ban' }
    };
    ...
    const cardsHtml = orders.map(o => {
        ...
        const custAddress = escapeHTML(o.customer ? o.customer.address : 'تسليم من الفرع / بدون عنوان');
        const custNotes = escapeHTML(o.customer && o.customer.notes ? o.customer.notes.trim() : '');
        const orderTime = new Date(o.createdAt || o.orderDate).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' });

        const items = o.items || [];
        const itemsHtml = items.map(i => {
            let sizeBadge = i.selectedSize && i.selectedSize.name ? `<span class="kds-size-badge">${escapeHTML(i.selectedSize.name)}</span>` : '';
            let addonsText = i.selectedAddons && i.selectedAddons.length ? 
        ...
        if (o.customer && o.customer.gpsLocation && o.customer.gpsLocation.mapUrl) {
            gpsButtonHtml = `<a href="${escapeHTML(o.customer.gpsLocation.mapUrl)}" target="_blank" class="kds-gps-btn"><i class="fa-solid fa-location-dot"></i> فتح الخريطة</a>`;
            ...
            gpsButtonHtml = `<a href="${mapUrl}" target="_blank" class="kds-gps-btn"><i class="fa-solid fa-location-dot"></i> فتح الخريطة</a>`;
        }

        const statusInfo = statusConfig[o.status] || { bg: '#A8792F', color: '#FFF', label: o.status, icon: 'fa-clock' };
                ...
                <div class="kds-order-card h-100">
                    <div class="kds-card-header d-flex align-items-center justify-content-between">
                        ...
                        <div class="kds-status-select-wrapper" style="background-color: ${statusInfo.bg}; color: ${statusInfo.color};">
                            ...
                            <select class="kds-status-select" onchange="updateOrderStatusOptimistic('${o._id}', this.value)">
                                ...
                                <option value="Ready" ${o.status === 'Ready' ? 'selected' : ''}>📦 جاهز بالـمطبخ</option>
                    ...
                    <div class="kds-card-body p-3">
                        <div class="kds-customer-box mb-3">
                            <div class="d-flex align-items-start justify-content-between gap-2">
                                <div>
                                    <h6 class="fw-bold mb-1 text-ink"><i class="fa-solid fa-user text-brass me-1"></i> ${realCustomerName}</h6>
                            ...
                            ${custNotes ? `<div class="kds-notes-alert mt-2"><i class="fa-solid fa-note-sticky text-amber me-1"></i> <strong>ملاحظة:</strong> ${custNotes}</div>` : ''}
                        </div>
                        ...
                        <div class="kds-items-box">
                            <div class="kds-items-header d-flex justify-content-between align-items-center mb-2 pb-1 border-bottom">
                                <span class="fw-bold text-ink small"><i class="fa-solid fa-utensils text-brass me-1"></i> الوجبات المطلوبة</span>
                                <span class="badge rounded-pill bg-dark text-warning">${items.length} أصناف</span>
                            </div>
                    ...
                    <div class="kds-card-footer p-3 border-top">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <span class="text-muted small fw-bold">إجمالي الحساب:</span>
                            <span class="kds-total-price">${o.totalPrice} <small class="fs-6 text-dark fw-bold">ج.م</small></span>
                        </div>

                        <div class="d-flex gap-2">
                            <button class="btn kds-btn-whatsapp flex-grow-1" onclick='openCustomerWhatsapp(${JSON.stringify(o)})' title="واتساب مباشر">
                                <i class="fa-brands fa-whatsapp fs-5"></i> <span>واتساب</span>
                            </button>
                            <button class="btn kds-btn-print" onclick='printThermalReceipt(${JSON.stringify(o)})' title="طباعة فاتورة">
                                <i class="fa-solid fa-print"></i> <span>طباعة</span>
                            </button>
                            ${isSuperAdmin ? `
                            <button class="btn kds-btn-delete" onclick="deleteOrderFromDB('${o._id}')" title="حذف">
